...
## ADR-013: 비동기 전용과 공통 TPS (2026-09-14)

사용자 요청에 따라 ADR-004의 동기/비동기 쌍 정책을 대체한다. 기존 비동기 구현을 기본
클라이언트 이름으로 통합하고 별칭을 제거했다. 각 송신과 재시도/리다이렉트는 동일
AsyncTokenBucket을 사용한다. 네트워크와 연결되는 DB 동기화 함수는 AsyncSession으로
변경한다. 로컬 파싱/모델/독립 DB 배치 함수는 유지한다. 자세한 계약은 [async-tps.md](async-tps.md).
