...
이 문서는 `pymois`가 행정안전부 지방행정 인허가정보를 어떤 규칙으로 구현하는지 정리합니다. 사용자용 요약은 `README.md`, 전체 목록은 `docs/api-list.md`와 `docs/response-fields.md`를 봅니다.
...
`pymois`는 이를 위해 다음 편의 메서드를 제공합니다.
...
- EPSG:5174 좌표가 있으면 WGS84 `(lon, lat)`를 계산해 `Coordinate`, `KatecPoint`, `Wgs84Point`, `StationCoordinates`와 `WGS84_LON`, `WGS84_LAT`로 추가

좌표/enum/type 공개 API는 [타입과 좌표 값 객체](docs/types-and-coordinates.md)에 정리합니다.

## 카탈로그 생성

`pymois/catalog.py`는 공식 첨부자료와 localdata 카탈로그를 바탕으로 생성된 정적 데이터입니다.
...
특히 단일 flat table 대신 공통 검색 필드는 `tb_place_master`, 업종별 특수 필드와 이력은 JSONB 기반 `tb_place_detail`에 저장하는 하이브리드 구조를 권장합니다.

## DB 적재 구현

`pymois.db`는 위 설계를 SQLAlchemy 2 ORM으로 구현합니다.

- `PlaceRecord`: `LocalDataRecord`를 DB 적재용으로 정규화하는 Pydantic 모델
- `PlaceMaster`: `mois_place_master`, 공통 검색/공간 검색 필드
- `PlaceDetail`: `mois_place_detail`, 업종별 특수 필드와 원본 JSONB
- `BatchSyncLog`: `mois_batch_sync_log`, 증분/파일 동기화 상태
- `create_postgis_schema()`: PostGIS 확장과 테이블 생성
- `upsert_places()`: `(service_slug, mng_no)` 기준 UPSERT

법정동코드, 도로명코드, 건물관리번호 연계 후보와 보강 흐름은 [PostgreSQL/PostGIS DB 적재](docs/database.md)에 정리했습니다.
